...
def consolidar_ventas(df_tienda, df_online, df_catalogo):
    ...
    df_tienda.columns = [
        'fecha_venta', 'hora', 'id_transaccion', 'sucursal',
        'nombre_producto', 'cantidad', 'precio_unitario',
        'vendedor', 'metodo_pago'
    ]

    df_online.columns = [
        'fecha_venta', 'id_transaccion', 'nombre_producto',
        'cantidad', 'precio_unitario', 'customer_email',
        'city', 'status', 'shipping_cost'
    ]
    ...
    df_tienda = pd.merge(
        df_tienda, df_catalogo,
        left_on="nombre_producto", right_on="producto",
        how="left"
    )

    df_online = pd.merge(
        df_online, df_catalogo,
        left_on="nombre_producto", right_on="producto",
        how="left"
    )
...
def calcular_metricas(df):
    ...
    df['semana_del_mes'] = df['fecha_venta'].apply(
        lambda x: (x.day + x.replace(day=1).weekday()) // 7 + 1
    )
